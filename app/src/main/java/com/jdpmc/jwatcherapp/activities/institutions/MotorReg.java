...
public class MotorReg extends AppCompatActivity implements View.OnClickListener{
    ...
    private static final String TAG = MotorReg.class.getSimpleName();
    private String path;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        notice.setText(new StringBuilder().append("Kindly snap the front view of the Motor Park").toString());


        rateid = (Spinner) findViewById(R.id.rateid);
        rateid.setOnItemSelectedListener(new ItemSelectedListener());

        setTitle("Register Motor Park");
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.button_upload:
                ...
                if (postPath == null) {
                    Toast.makeText(this, "please snap the front of the motor Park", Toast.LENGTH_SHORT).show();
                } else if (input_comment.length() == 0) {
                    comment.setError("Park name is required");
                }else if (input_comment2.length() == 0) {
                    comment2.setError("Office Name is required");
                    ...
                    String mcontactphone = Objects.requireNonNull(input_comment3.getText()).toString();
                        ...
                        submitDetails(mname, mstate, mcontactname, mcontactphone);
                }
                break;
            case R.id.selectImage:
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    if ((ContextCompat.checkSelfPermission(getApplicationContext(), Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED)) {
                        ActivityCompat.requestPermissions(MotorReg.this, new String[] { Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.CAMERA, Manifest.permission.WRITE_EXTERNAL_STORAGE }, 0);
                    } else {
                        launchImagePicker();
                    }
                } else {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == 0) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED
                ...
                Toast.makeText(MotorReg.this, "Permission denied, the permissions are very important for the apps usage", Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    private void submitDetails(String sname, String sstate, String scontactname, String scontactphone){
        ...
        RequestBody address = createPartFromString(ssender);
        RequestBody sendername = createPartFromString(susername);

        Call<Void> call = userService.nsreg(typr, name, state, contactname, contactphone, address, latitude, longitude, body);
        call.enqueue(new Callback<Void>() {
            ...
            public void onResponse(@NonNull Call<Void> call, @NonNull Response<Void> response) {
                if (response.isSuccessful()) {
                    ...
                    emptyInputEditText();
                    Toast.makeText(MotorReg.this, "Successfully Sent", Toast.LENGTH_SHORT).show();
                } else {
                    progress.setVisibility(View.GONE);
                    Toast.makeText(MotorReg.this, "Error Occured", Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onFailure(@NonNull Call<Void> call, @NonNull Throwable t) {
                progress.setVisibility(View.GONE);
                Toast.makeText(MotorReg.this, "Error Occured " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
