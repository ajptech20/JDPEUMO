...
public class UploadMediaActivity extends AppCompatActivity implements View.OnClickListener {
            ...
            case R.id.selectImage:
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                    if ((ContextCompat.checkSelfPermission(getApplicationContext(), Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED)) {
                        ActivityCompat.requestPermissions(UploadMediaActivity.this, new String[] { Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.CAMERA, Manifest.permission.WRITE_EXTERNAL_STORAGE }, 0);
                    } else {
                        launchImagePicker();
                    }
                } else {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == 0) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED
                ...
                Toast.makeText(UploadMediaActivity.this, "Permission denied, the permissions are very important for the apps usage", Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    private void submitDetails(String comment){
        progress.setVisibility(View.VISIBLE);
        Service userService = DataGenerator.createService(Service.class, BASE_URL);

        // create part for file (photo, video, ...)
        MultipartBody.Part body = prepareFilePart("picture");
        // create a map of data to pass along
        RequestBody m_comment = createPartFromString(comment);

        HashMap<String, RequestBody> map = new HashMap<>();
        map.put("comment", m_comment);

        String longitude = PreferenceUtils.getLocationLongitude(getApplicationContext());
        String latitude = PreferenceUtils.getLocationLatitude(getApplicationContext());
        String phonenumber = PreferenceUtils.getPhoneNumber(getApplicationContext());

        Call<Void> call = userService.sendComment(map, body, phonenumber, latitude, longitude);
        call.enqueue(new Callback<Void>() {
            ...
            public void onResponse(@NonNull Call<Void> call, @NonNull Response<Void> response) {
                if (response.isSuccessful()) {
                    ...
                    emptyInputEditText();
                    Toast.makeText(UploadMediaActivity.this, "comment successfully uploaded", Toast.LENGTH_SHORT).show();
                } else {
                    progress.setVisibility(View.GONE);
                    Toast.makeText(UploadMediaActivity.this, "error uploading comment", Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onFailure(@NonNull Call<Void> call, @NonNull Throwable t) {
                progress.setVisibility(View.GONE);
                Toast.makeText(UploadMediaActivity.this, "error uploading comment " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
