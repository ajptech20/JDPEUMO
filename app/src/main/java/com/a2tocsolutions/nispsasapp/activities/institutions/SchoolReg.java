...
import static com.a2tocsolutions.nispsasapp.utils.Constants.Reg_School;

public class SchoolReg extends AppCompatActivity implements View.OnClickListener {
    ...
    private static final String TAG = SchoolReg.class.getSimpleName();
    private String path;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.school_reg);

        ButterKnife.bind(this);
        if (getSupportActionBar() != null) {
            getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        }
        notice.setText(new StringBuilder().append("Kindly snap the front view of the School").toString());
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.button_upload:
                ...
                if (postPath == null) {
                    Toast.makeText(this, "please snap the front of the School", Toast.LENGTH_SHORT).show();
                } else if (input_comment.length() == 0) {
                    comment.setError("School name is required");
                }else if (input_comment2.length() == 0) {
                    comment2.setError("Officer Name is required");
                }else if (input_comment3.length() == 0) {
                    comment3.setError("Phone number is required");
                  }else if (input_comment4.length() == 0) {
                    comment3.setError("Address is required");
                }else{
                    String mname  = Objects.requireNonNull(input_comment.getText()).toString();
                    String mstate = rateid.getSelectedItem().toString();
                    String mcontactname = Objects.requireNonNull(input_comment2.getText()).toString();
                    String mcontactphone = Objects.requireNonNull(input_comment3.getText()).toString();
                    String maddress = Objects.requireNonNull(input_comment4.getText()).toString();

                        submitDetails(mname, mstate, mcontactname, mcontactphone, maddress);
                }
                break;
            case R.id.selectImage:
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    if ((ContextCompat.checkSelfPermission(getApplicationContext(), Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED)) {
                        ActivityCompat.requestPermissions(SchoolReg.this, new String[] { Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.CAMERA, Manifest.permission.WRITE_EXTERNAL_STORAGE }, 0);
                    } else {
                        launchImagePicker();
                    }
                } else {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == 0) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED
                ...
                Toast.makeText(SchoolReg.this, "Permission denied, the permissions are very important for the apps usage", Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    private void submitDetails(String sname, String sstate, String scontactname, String scontactphone, String saddress){
        progress.setVisibility(View.VISIBLE);
        Service userService = DataGenerator.createService(Service.class, Reg_School);
        ...
        String ssender = PreferenceUtils.getPhoneNumber(getApplicationContext());
        String susername = PreferenceUtils.getUsername(getApplicationContext());
        String typer= "School";
        //convert String to RequestBody to pass along with Image
        RequestBody name = createPartFromString(sname);
        RequestBody typr = createPartFromString(typer);
        RequestBody state = createPartFromString(sstate);
        RequestBody contactname = createPartFromString(scontactname);
        RequestBody contactphone = createPartFromString(scontactphone);
        RequestBody longitude = createPartFromString(slongitude);
        RequestBody latitude = createPartFromString(slatitude);
        RequestBody address = createPartFromString(saddress);



        Call<Void> call = userService.nsreg(typr, name, state, contactname, contactphone, latitude, longitude, address, body);
        call.enqueue(new Callback<Void>() {
            ...
            public void onResponse(@NonNull Call<Void> call, @NonNull Response<Void> response) {
                if (response.isSuccessful()) {
                    ...
                    emptyInputEditText();
                    Toast.makeText(SchoolReg.this, "Successfully Sent", Toast.LENGTH_SHORT).show();
                } else {
                    progress.setVisibility(View.GONE);
                    Toast.makeText(SchoolReg.this, "Error Occured", Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onFailure(@NonNull Call<Void> call, @NonNull Throwable t) {
                progress.setVisibility(View.GONE);
                Toast.makeText(SchoolReg.this, "Error Occured " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
